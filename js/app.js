...
class SoundGraffiti {
    ...
    showHelp() {
        ...
        helpBody.innerHTML = `
            ...
            <div class="help-section">
                <h3>⚠️ 免責事項</h3>
                <p>このアプリケーションは可能な限り正確な測定を目指して開発していますが、計測値の完全な正確性は保証できません。本ソフトウェアは「現状のまま」で提供され、明示的または黙示的を問わず、いかなる保証もありません。</p>
                <p>本ツールの使用により生じたいかなる損害（データの損失、業務の中断、利益の損失、その他の金銭的損害を含むがこれらに限定されません）についても、開発者は一切の責任を負いません。</p>
                <p>プロフェッショナルな用途での使用は、専用のハードウェア/ソフトウェアによる検証を強く推奨します。本ツールのご利用は完全に自己責任でお願いいたします。</p>
            </div>
        `;
